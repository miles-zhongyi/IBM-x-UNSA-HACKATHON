...
const InfoChip = ({ icon: Icon, label, value }) => (
  <div className="flex items-start gap-3 p-3 rounded-xl bg-[#F7FFFD]">
    ...
    <div>
      <div className="text-xs uppercase tracking-wider text-[#4B7A73]">{label}</div>
    ...
    <div className="space-y-6">
      ...
      <div className="card-soft p-6 flex items-center gap-6 flex-wrap">
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <div className="text-sm text-[#4B7A73] mt-1">{patient.age} years • {patient.sex} • {patient.status}</div>
        ...
        <div className="text-right">
          <div className="text-xs uppercase tracking-wider text-[#4B7A73]">Pending questions</div>
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview" className="pt-6">
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="card-soft p-6">
              <h3 className="font-[Outfit] text-lg font-semibold text-[#2F5D57] mb-4">Contact</h3>
            ...
            <div className="card-soft p-6">
              <h3 className="font-[Outfit] text-lg font-semibold text-[#2F5D57] mb-4">Quick facts</h3>
              <div className="grid grid-cols-2 gap-3">
                <InfoChip icon={Pill} label="Medications" value={patient.medications.join(", ")} />
                <InfoChip icon={AlertCircle} label="Allergies" value={patient.allergies.join(", ")} />
                <InfoChip icon={Stethoscope} label="Conditions" value={patient.conditions.join(", ")} />
                <InfoChip icon={Calendar} label="Last activity" value={fmtDate(patient.last_activity)} />
              </div>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="history" className="pt-6">
          <div className="card-soft p-6">
            <h3 className="font-[Outfit] text-lg font-semibold text-[#2F5D57] mb-4">Timeline</h3>
            <ol className="border-l-2 border-[#A7E3D4] pl-6 space-y-6">
              ...
              {documents.length === 0 && <li className="text-sm text-[#4B7A73]">No history yet.</li>}
            </ol>
          </div>
        </TabsContent>
        ...
        <TabsContent value="documents" className="pt-6">
          <div className="grid md:grid-cols-2 gap-4">
            {documents.length === 0 && <div className="card-soft p-8 text-center text-[#4B7A73]">No documents uploaded yet.</div>}
            {documents.map((d) => (
        ...
        <TabsContent value="conversations" className="pt-6">
          <div className="card-soft p-6">
            {threads.length === 0 ? (
              <p className="text-sm text-[#4B7A73] text-center py-8">No conversations yet.</p>
            ) : (
              <ul className="divide-y divide-[#C2EBE1]/60">
                {threads.map((t) => (
                  <li key={t.id} className="py-3 flex items-center gap-4">
                    <div className={`w-2 h-2 rounded-full ${t.status === "urgent" ? "bg-[#E05A5A]" : t.status === "unanswered" ? "bg-[#E5A832]" : "bg-[#5BB9A6]"}`} />
                    <div className="flex-1">
                      <div className="font-semibold text-[#2F5D57]">{t.subject}</div>
                      <div className="text-xs text-[#4B7A73]">{fmtDate(t.last_message_at)}</div>
                    </div>
                    <span className="text-xs text-[#4B7A73] capitalize">{t.status}</span>
                  </li>
                ))}
              </ul>
            )}
        ...
        <TabsContent value="ai" className="pt-6">
          <div className="card-soft p-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-[Outfit] text-lg font-semibold text-[#2F5D57]">AI Health Summary</h3>
            </div>
            <p className="text-base text-[#1A332F] leading-relaxed">{patient.ai_summary || "No AI summary yet."}</p>
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
              <InfoChip icon={Pill} label="Medications" value={patient.medications.join(", ") || "None"} />
              <InfoChip icon={Stethoscope} label="Conditions" value={patient.conditions.join(", ") || "None"} />
              <InfoChip icon={AlertCircle} label="Allergies" value={patient.allergies.join(", ") || "None"} />
            </div>
            <p className="mt-4 text-xs text-[#4B7A73] italic">AI summaries are educational and informational only — not a substitute for clinical judgment.</p>
          </div>
        </TabsContent>
      </Tabs>
    </div>
